<script lang="ts">
  ...
  onMount(async () => {
    ...
    if (lineChartCanvas && data.donacionesPorMes.length > 0) {
      const meses = data.donacionesPorMes.map(m => {
...
<!-- Hero Section -->
<section class="relative pt-12 pb-32 md:pt-16 md:pb-40 overflow-hidden">
  ...
  <div class="absolute bottom-0 left-0 w-80 h-80 bg-primary-600/20 rounded-full blur-3xl translate-y-1/2 -translate-x-1/3 pointer-events-none"></div>
  <!-- Subtle pattern -->
...
<section class="py-16 relative overflow-hidden">
  ...
  <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="text-center mb-12">
      <div class="inline-flex items-center gap-2 px-3 py-1 bg-accent-100 rounded-full text-accent-700 text-xs font-semibold uppercase tracking-wider mb-4">
    ...
    {:else}
      {#if data.topDonantes.length >= 3}
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-6 mb-10 max-w-4xl mx-auto md:items-end">
          <!-- 2nd Place -->
          <div class="order-2 md:order-1 animate-slide-up" style="animation-delay: 0.2s">
            <div class="relative card p-6 pt-10 text-center border-2 border-slate-200/80 hover:border-slate-300 group transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
              <div class="absolute -top-5 left-1/2 -translate-x-1/2">
                <div class="w-10 h-10 bg-gradient-to-br from-slate-300 to-slate-400 rounded-full flex items-center justify-center shadow-lg ring-4 ring-white">
                  <span class="text-base font-display font-bold text-white">2</span>
                </div>
              </div>
              <div class="w-14 h-14 mx-auto mb-3 bg-slate-100 rounded-full flex items-center justify-center text-2xl group-hover:scale-110 transition-transform duration-300">🥈</div>
              <h3 class="font-display font-bold text-base text-slate-900 leading-tight group-hover:text-primary-700 transition-colors truncate">
                {data.topDonantes[1].nombreNegocio || data.topDonantes[1].nombre}
              </h3>
              {#if data.topDonantes[1].nombreNegocio}
                <p class="text-xs text-slate-500 mt-0.5 truncate">{data.topDonantes[1].nombre}</p>
              {/if}
              <div class="mt-2">
                <Badge variant={data.topDonantes[1].tipo === 'EMPRESA' ? 'info' : 'default'} size="sm">
                  {data.topDonantes[1].tipo === 'EMPRESA' ? 'Empresa' : 'Persona'}
                </Badge>
              </div>
          ...
          <!-- 1st Place -->
          <div class="order-1 md:order-2 animate-slide-up" style="animation-delay: 0.1s">
            <div class="relative card p-7 pt-12 text-center border-2 border-accent-300 bg-gradient-to-b from-accent-50/60 via-white to-white hover:border-accent-400 group transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl md:pb-8">
              <div class="absolute -top-5 left-1/2 -translate-x-1/2">
                <div class="relative">
                  <div class="w-10 h-10 bg-gradient-to-br from-accent-400 to-yellow-500 rounded-full flex items-center justify-center shadow-lg ring-4 ring-white">
                    <span class="text-base font-display font-bold text-white">1</span>
                  </div>
                  <div class="absolute -top-1 -right-1 w-4 h-4 bg-accent-400 rounded-full animate-ping opacity-40"></div>
                </div>
              </div>
              <div class="w-20 h-20 mx-auto mb-4 bg-gradient-to-br from-accent-100 to-accent-50 rounded-full flex items-center justify-center text-4xl group-hover:scale-110 transition-transform duration-300 ring-4 ring-accent-200/50">
                🏆
              </div>
              <h3 class="font-display font-bold text-xl text-slate-900 leading-tight group-hover:text-primary-700 transition-colors">
                {data.topDonantes[0].nombreNegocio || data.topDonantes[0].nombre}
              </h3>
              {#if data.topDonantes[0].nombreNegocio}
                <p class="text-sm text-slate-500 mt-0.5">{data.topDonantes[0].nombre}</p>
              {/if}
              <div class="mt-2">
                <Badge variant={data.topDonantes[0].tipo === 'EMPRESA' ? 'info' : 'default'}>
                  {data.topDonantes[0].tipo === 'EMPRESA' ? 'Empresa' : 'Persona'}
                </Badge>
              </div>
          ...
          <!-- 3rd Place -->
          <div class="order-3 animate-slide-up" style="animation-delay: 0.3s">
            <div class="relative card p-6 pt-10 text-center border-2 border-orange-200/80 hover:border-orange-300 group transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
              <div class="absolute -top-5 left-1/2 -translate-x-1/2">
                <div class="w-10 h-10 bg-gradient-to-br from-orange-400 to-orange-500 rounded-full flex items-center justify-center shadow-lg ring-4 ring-white">
                  <span class="text-base font-display font-bold text-white">3</span>
                </div>
              </div>
              <div class="w-14 h-14 mx-auto mb-3 bg-orange-50 rounded-full flex items-center justify-center text-2xl group-hover:scale-110 transition-transform duration-300">🥉</div>
              <h3 class="font-display font-bold text-base text-slate-900 leading-tight group-hover:text-primary-700 transition-colors truncate">
                {data.topDonantes[2].nombreNegocio || data.topDonantes[2].nombre}
              </h3>
              {#if data.topDonantes[2].nombreNegocio}
                <p class="text-xs text-slate-500 mt-0.5 truncate">{data.topDonantes[2].nombre}</p>
              {/if}
              <div class="mt-2">
                <Badge variant={data.topDonantes[2].tipo === 'EMPRESA' ? 'info' : 'default'} size="sm">
                  {data.topDonantes[2].tipo === 'EMPRESA' ? 'Empresa' : 'Persona'}
                </Badge>
              </div>
      ...
      {#if data.topDonantes.length > 3}
        <div class="max-w-2xl mx-auto space-y-2.5">
          {#each data.topDonantes.slice(3) as donante, i}
            <div
              class="group flex items-center gap-4 bg-white/80 backdrop-blur-sm rounded-xl border border-slate-200/60 px-5 py-3.5 hover:border-primary-300 hover:bg-white hover:shadow-md transition-all duration-300 animate-slide-up"
              style="animation-delay: {(i + 4) * 0.06}s"
            >
              <span class="w-8 h-8 rounded-full bg-slate-100 group-hover:bg-primary-50 flex items-center justify-center text-sm font-bold text-slate-500 group-hover:text-primary-700 transition-colors shrink-0">
                {i + 4}
              </span>
              <div class="flex-1 min-w-0">
                <span class="font-medium text-slate-900 block truncate">{donante.nombreNegocio || donante.nombre}</span>
                {#if donante.nombreNegocio}
                  <span class="text-xs text-slate-500 block truncate">{donante.nombre}</span>
                {/if}
              </div>
              <Badge variant={donante.tipo === 'EMPRESA' ? 'info' : 'default'} size="sm">
      ...
      <div class="text-center mt-10">
        <a href="/buscar" class="btn btn-outline group">
...
<section class="py-12">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <h2 class="section-title mb-8 text-center">Estadísticas de Recaudación</h2>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    {:else}
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {#each data.ultimosAportes as aporte, i}
          <div
            class="card p-5 hover:border-primary-200 transition-all animate-slide-up"
            style="animation-delay: {i * 0.05}s"
          >
            <div class="flex items-start justify-between mb-3">
              <div>
                <h4 class="font-medium text-slate-900">{aporte.donante}</h4>
                <p class="text-xs text-slate-500">{formatDate(aporte.fecha)}</p>
              </div>
...
<section class="py-16 bg-gradient-to-br from-primary-800 to-primary-950">
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
    <h2 class="text-3xl font-display font-bold text-white mb-4">
      ¿Quieres contribuir al desarrollo de Monterrey?
    </h2>
