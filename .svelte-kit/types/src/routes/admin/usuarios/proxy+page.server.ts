...
import { userCreateSchema, userPasswordResetSchema, userToggleSchema } from '$lib/validations';
...
export const actions = {
  create: async ({ request, locals }: import('./$types').RequestEvent) => {
    ...
    const payload = {
      ...
      role: (formData.get('role') ?? 'ADMIN') as string,
    };
  ...
  toggleActive: async ({ request, locals }: import('./$types').RequestEvent) => {
    const user = locals.user;
